...
- estimating thresholds when to accept the measures
    ...
    - Robustness: Report the rates of false positives for 3 different thresholds here! Report the rates of false negatives as well! => this can be done in the B4!
        - geschrieben!
        - => see ![](results/robustness_firms_thresholds.tex)
- Robustness: compare the matching with the real share of american employees working in public companies! => even though this probably doesn't say much because there might be other reasons for why they are less represented in the data, it gives an impression of how far off I am!
    - According to @Schlingemann non-farm publically held domestic companies accounts for roughly 20% of employment in 2019 in the US. This number has decreased a lot since the 1970s, where it was still at 35%.
    - The matching algorithm however only matches X% of the database. => see ![](results/firm_match_propensity.tex)
...
- Robustness? #incertainty
    - Show that a set of companies thought of as "Tech" are actually in the "Tech" category! => take them from the database selling/strimling used! fortune 500 technology!
